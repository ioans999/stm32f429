...
#endif 		
    tmpreg = ETH_ReadPHYRegister(PHYAddress, 2); // PHY ID
    RegValue = ETH_ReadPHYRegister(PHYAddress, 3); //PHY ID
		tmpreg <<= 16;
		tmpreg |= RegValue&0xfff0;
		if(tmpreg != 0x7c0f0) {UART_printf("Read PHY ID - FAILED\n\r"); return ETH_ERROR;}
		UART_printf("Read PHY ID (LAN8720) - OK\n\r",tmpreg);
    /* Read the result of the auto-negotiation */
