...
void ETH_BSP_Config(void)
{
  ...
  if(ETH_ReadPHYRegister(DP83848_PHY_ADDRESS, PHY_BSR) & 4)
  {
...
void ETH_GPIO_Config(void)
{
  ...
  /* Enable GPIOs clocks */
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA | RCC_AHB1Periph_GPIOB | RCC_AHB1Periph_GPIOD |
...
/* Ethernet pins configuration ************************************************/
   /*
        ...
        ETH_MII_CRS ----------------------> PH2 cut
        ETH_MII_COL ----------------------> PH3 cut
        ETH_MII_RX_ER --------------------> PI10 cut
        ETH_MII_RXD2 ---------------------> PH6 cut
        ETH_MII_RXD3 ---------------------> PH7 cut
        ETH_MII_TX_CLK -------------------> PC3 cut
        ETH_MII_TXD2 ---------------------> PC2 cut
        ETH_MII_TXD3 ---------------------> PB8 cut
...
/*  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;
  ...
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
	...
	GPIO_ResetBits(GPIOD, 1<<11);
	for(i=0;i<100;i++) ;
	GPIO_SetBits(GPIOD, 1<<11);
	for(i=0;i<1000;i++) ;
}
